...
//	}
	...
	public String transferAmount(TransferRequestDTO request) {
		...
		Account fromAccount = repo.findById(request.getFromAccountNumber())
				.orElseThrow(() -> new RuntimeException("Sender Account does not Exist"));

		Account toAccount = repo.findById(request.getToAccountNumber())
				.orElseThrow(() -> new RuntimeException("Receiver Account does not Exist"));

		// check pin
		if (!fromAccount.getPin().equals(request.getPin())) {

			throw new RuntimeException("PIN is inncorect");
		}
		...
		transactionForSender.setDescription("Sallary to employee");
		...
		transactionForReceiver.setDescription("Salary");

		Trepo.save(transactionForSender);
		Trepo.save(transactionForReceiver);

		return "Transfer successful from Account " + fromAccount.getAccount_number() + " to Account "
